...
def corr_person(): #Function is used to show user all oppurtunities
    ...
    while len(person_list) > 1:
        ...
        while choice_made == False:
            choice_person = input() 
            if choice_person == "1" or choice_person == "2" or choice_person == "3" or choice_person == "4" or choice_person == "5" or choice_person == "6" or choice_person == "7": #I want the user to be able to pick the same person as many times as they want.
            ...
            person_list = check_list(person_list,choice_person) #Check list for person and remove

        if len(person_list) > 1: #Ask for more if hasnt reached exit benchmark
            print(" And?, Anyone Else you might be able to be freinds with?")
    finished_choosing()
   
   
   
def finished_choosing(): #A Function to end the game for player
